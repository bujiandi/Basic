...
extension Sequence {
    ...
    @inlinable public func firstMap<U>(`where` predicate: (Element) throws -> U?) rethrows -> U? {
        return (try first { try predicate($0) != nil }) as? U
    }
}
...
extension Collection {
    ...
    @inlinable public func split(elementCount:Int) -> [[Iterator.Element]] {
        if elementCount <= 0 {
            fatalError("elementCount mast bigger than 0")
        }
    ...
    @inlinable public func element(at index:Index) -> Iterator.Element? {
        if !(startIndex..<endIndex).contains(index) { return nil }
        return self[index]
    }
    
    @inlinable public func set<T>(_ includeElement: (Iterator.Element) -> T) -> Set<T> where T:Hashable {
        var set = Set<T>()
    ...
    @inlinable public func joined(separator:String, includeElement:(Iterator.Element) -> String = { unwrapOptionalToString($0) }) -> String {
        var result:String = ""
        for item:Iterator.Element in self {
    ...
    @inlinable public func find(where includeElement: (Iterator.Element) -> Bool) -> Iterator.Element? {
...
//    
//    
//    public func group(_ count:Int) -> [[Element]] {
//        self.split(maxSplits: <#T##Int#>, omittingEmptySubsequences: <#T##Bool#>, whereSeparator: <#T##(Element) throws -> Bool#>)
//    }
